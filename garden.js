...
function buildJob(s){jobScenery.traverse(o=>{if(o.isMesh){if(![unit,sphere,cylinder].includes(o.geometry))o.geometry.dispose();if(o.geometry.type==='PlaneGeometry'){o.material.map?.dispose();o.material.dispose();}}});jobScenery.clear();gateVisual=null;
...
document.querySelector('#jobs').onclick=()=>{const s=window.Wheelbarrow.getState();menuWasPaused=s.paused;if(!s.paused)window.Wheelbarrow.toggle();menu.hidden=false;const list=document.querySelector('#job-list');list.replaceChildren();s.levels.forEach((l,i)=>{const b=document.createElement('button');b.disabled=i>s.save.unlocked;const best=s.save.best[i];b.textContent=(i+1)+'. '+l.name+' · '+(b.disabled?'Locked':best?best.delivered+'/6 delivered · '+best.spills+' spills':'Deliver '+l.target+'/6');b.onclick=()=>{menu.hidden=true;window.Wheelbarrow.selectLevel(i);canvas.focus();};const p=document.createElement('p');p.textContent=l.brief;list.append(b,p);});list.querySelector("button:not(:disabled)")?.focus();};
document.querySelector('#close-jobs').onclick=closeJobs;
document.querySelector('#next-job').onclick=()=>{const s=window.Wheelbarrow.getState();if(s.levelIndex<2)window.Wheelbarrow.selectLevel(s.levelIndex+1);else document.querySelector('#jobs').click();};
...
window.renderGame=s=>{
 ...
 const now=performance.now();frames++;if(now-fpsStart>1000){canvas.dataset.fps=String(Math.round(frames*1000/(now-fpsStart)));canvas.dataset.drawCalls=String(renderer.info.render.calls);frames=0;fpsStart=now;}if(now-lastHUD>90){lastHUD=now;els.delivered.innerHTML=s.delivered+'<span>/6</span>';els.time.textContent=Math.floor(s.elapsed/60)+':'+String(Math.floor(s.elapsed%60)).padStart(2,'0');els.spills.textContent=s.spills;els.paused.hidden=!s.paused||!menu.hidden;els.bars.forEach((e,i)=>e.classList.toggle('done',i<s.delivered));els.dot.style.transform=`translate(${s.mouseX*22}px,${s.mouseY*22}px)`;

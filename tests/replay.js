...
async function campaign(){
 for(let job=mode==='expansion'?3:0;job<api.getState().levels.length;job++){api.selectLevel(job);mouse(0,0);if(api.getState().levelIndex!==job)throw Error('Job locked after completion');panel.textContent='RUNNING campaign: job '+(job+1);
 for(const [x,y] of api.getState().level.route){await drive(x,y);if(job===2&&x===795){keys(['e']);keys([]);await wait(500);}}
 keys([]);await wait(2200);if(api.getState().level.kind==='gary'){keys(['e']);keys([]);}else mouse(0,-1);const start=performance.now();while(api.getState().delivered<api.getState().level.count&&performance.now()-start<22000)await wait(100);mouse(0,0);const s=api.getState();if(s.delivered!==s.level.count||s.spills)throw Error('Job '+(job+1)+' delivered '+s.delivered+' spills '+s.spills);console.log('PASS campaign job '+(job+1)+' '+s.delivered+' delivered; zero spills');
 }panel.textContent='PASS '+mode+': all requested jobs · zero spills';api.toggle();
}
async function run(){if(mode==='campaign'||mode==='expansion')return campaign();api.reset();mouse(0,0);panel.textContent='RUNNING: '+mode+' browser input replay';

...
## Revision 5 — Garden estate jobs, 24 September 2026
The user's request to expand supersedes the earlier no-campaign scope gate. The accepted visual style is now a standing requirement for all future levels (also recorded in AGENTS.md).

Three sequential jobs share the garden estate: Garden Job (four required, wide route or shortcut); Builders’ Passage (five required, south closure and staggered collision stacks); Muddy Service Run (six required, central closure, mud drag and a stopped E-operated gate). Physics and render obstacles use the same authored rectangles. The gate stays open until restarting the job. E still recovers cargo elsewhere. The muddy patch uses textured ground, slows movement, and adds bumps at unsafe speed. A Jobs board pauses play, presents briefings, and allows replay of unlocked jobs. Delivery acceptance unlocks the next job without forcing departure; optional remaining bricks improve the saved result. Unlocks and best delivery/spill result persist in local browser storage, with graceful fallback when storage is unavailable. Best results prioritize delivered count, then fewer spills. No timer limit or forced reset.

These are three obstacle layouts within chapter one's estate, not three different biomes. Separate construction and farm chapters remain future expansion targets using the same art standard.

## Revision 6 — Odd Jobs: three new destinations
The user asked to move beyond bricks in a garden and explicitly recalled construction sites and drunk passengers. Re-read Game Ideas Planning and recorded the full Wheelbarrow direction in ORIGINAL_VISION.md.

Now six jobs across four actual scene layouts: three existing garden jobs, Timber for the Tower (construction foundations, crane, barriers; three timbers), Get Gary Home (pub and village lane; one self-leaning passenger), The Biggest Pumpkin (barn, hay stacks, creek bridge and market; one rolling pumpkin). New destinations are immediately selectable; the garden's original sequential unlocks and records are preserved. Each destination owns its start, bay, collision layout, rough/muddy areas and authored route. Cargo counts, objective messages, mini-map and completion UI follow job data.

Gary leans periodically and induces roll/lateral load forces. E helps him off only when stopped in the home bay; if he tumbles, E recovers him nearby. Lumber increases turning imbalance and reduces tray friction. Pumpkin has less friction and visible rolling. All can spill and be recovered without forced restart. Shared textured materials, warm light and contact shadows remain the art standard. Photo snapshots exclude hidden destinations and inactive cargo.

See ORIGINAL_VISION.md for the remaining original cargo and equipment ideas, and explicit physics limitations.

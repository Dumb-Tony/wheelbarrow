...
## 24 September 2026 — three garden estate jobs
Deterministic steering/throttle regressions complete all three authored routes with 6/6 and zero spills, including the builders’ slalom and muddy service gate. Closed gate blocks movement; stopping and E opens it; restart closes it. Locked job selection is rejected. Saved unlock and six-brick record restore into a fresh game instance. Existing aggressive spill, complete recovery route, pause/reset, arrows and visible tilt regressions still pass. These are automated simulations, not human feel tests.
Browser campaign input replay passed all three jobs: 18/18 bricks, zero spills, including the gate interaction. This used real keyboard/pointer events and the animation loop, not position teleportation. Reloaded the ordinary page and verified all three saved six-brick, zero-spill results on the Jobs board. Visually inspected the route, gate and board. Human feel testing remains separate.
Final UI/render checks: Jobs selection starts the requested job; Escape returns from the board to play; board fits 800 x 600 and temporary viewport reset. Job two ray-traced photo accumulated 261 samples with no JavaScript errors and returned to play. Scene-specific geometry is disposed when switching jobs; shared materials remain in use.

## Odd Jobs expansion — 24 September 2026
New deterministic complete routes: construction 3/3 timbers, village 1/1 Gary, farm 1/1 pumpkin, all without spills. Each new cargo was then deliberately tipped out, recovered using E, and delivered on its full route. Gary changes roll without mouse input and cannot disembark outside HOME. Existing garden routes, recovery, gate, arrow mapping, visible tilt and persistence tests remain passing. New free destinations do not bypass the garden unlock chain.

Actual browser input replay on the three new maps: construction 3/3, village 1/1, farm 1/1; all zero spills and no JavaScript errors. This uses dispatched keyboard/pointer inputs and the live animation loop, not teleportation. Visually checked the construction scene and animated Gary in the village. This is automated route testing plus visual inspection, not an external human feel test.
Post-expansion garden browser regression: 6/6 bricks, zero spills, 46 seconds. Reloaded Jobs board retains the new destination records. Visually inspected the farm and pumpkin, shared material style, map-specific route/bay and cargo-specific HUD.
Final checks: farm ray-traced view reached 779 samples, showed only the farm and active pumpkin, returned to play without errors. Six-job board scrolls and selects Gary at 800 x 600; viewport restored. Switching farm to village after photo mode displayed correct cargo, controls and destination with no JavaScript errors.
